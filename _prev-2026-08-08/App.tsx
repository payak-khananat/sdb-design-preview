          ...
          {activeTabs.map(t => (
            ...
            >
              <span className="ds-nav-icon">
                ...
                {t.id === 'admin' && <ShieldCheck size={24} />}
                {t.id === 'operator' && <Settings size={24} />}
              </span>
      ...
      <div className="mobile-frame">
        ...
        <main className="app-main-content">
          ...
          <section className={`screen-view${screen === 'screen-approver-list' ? ' active' : ''}`}>
            ...
            {(approverFilter === 'all' || approverFilter === 'pending') && leaveCards.filter(c => c.status === 'pending').length > 0 && (
              <div className="apv-section">
                <div className="apv-section-header">
                  ...
                  <span className="apv-section-badge orange">{leaveCards.filter(c => c.status === 'pending').length} รายการ</span>
                </div>
                <div className="apv-card-list">
                  {leaveCards.filter(c => c.status === 'pending').map(card => (
                    <div key={card.id} className="apv-card apv-card--pending">
                      ...
                      <div className="apv-card-submitted">ยื่นเมื่อ {card.submitted}</div>
                      {/* Actions — prominent */}
            ...
            {/* Processed section */}
            {(approverFilter === 'all' || approverFilter === 'approved' || approverFilter === 'rejected') && leaveCards.filter(c => c.status !== 'pending').length > 0 && (
              <div className="apv-section">
                <div className="apv-section-header">
            ...
            {/* Empty state */}
            {leaveCards.filter(c => approverFilter === 'all' || c.status === approverFilter).length === 0 && (
          ...
          <section className={`screen-view${screen === 'screen-approver-create' ? ' active' : ''}`}>
            ...
            <div className="apv-step-block">
              ...
              <div className="form-row-2">
                <div className="form-group-mobile">
                  <label className="form-label required">ตั้งแต่วันที่</label>
                  <input type="date" className="form-input" defaultValue="2026-08-18" />
                </div>
                <div className="form-group-mobile">
                  <label className="form-label required">ถึงวันที่</label>
                  <input type="date" className="form-input" defaultValue="2026-08-18" />
                </div>
              </div>
              <div className="form-group-mobile">
                ...
                <div key={card.id} className={`ll-card ll-card--${card.status}`}>
                  ...
                  {card.status === 'pending' && (
                    <div className="ll-card-actions">
                      ...
                      <button className="ll-btn-view" style={{ background: '#28BD0A', color: '#fff', border: 'none' }} onClick={() => { setLeaveCards(prev => prev.map(c => c.id === card.id ? { ...c, status: 'approved' as const, updated: 'Operator' } : c)); showToast(`อนุมัติคำขอลา ${card.id} เรียบร้อยแล้ว`) }}><Check size={13} /> อนุมัติ</button>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className={`bottom-sheet-card${approvalSheet ? ' active' : ''}`}>
          ...
          <div className="sheet-actions-row">
            ...
            <button className="btn-sheet-submit green" onClick={confirmApprove}>ยืนยันอนุมัติ</button>
          </div>
        </div>
